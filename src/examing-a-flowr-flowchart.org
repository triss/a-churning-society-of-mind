...
* Contents                                                         :toc_3_gh:
 - [[#236---guardian-illustrated][236 - Guardian Illustrated]]
   - [[#modules][Modules]]
     ...
     - [[#textrandkeyphraseextractor][TextRandKeyPhraseExtractor]]
     - [[#flickr][Flickr]]
...
#+BEGIN_SRC clojure :tangle ../babel/src/a_churning_society_of_mind/guardian_illustrated.cljc :noweb yes :mkdirp yes :padline no
  ...
  (->> (guardian/search "Corbyn")
       ...
       (key-phrase/rank)
       (map #(-> % key-phrase/ranked-word->sentence))
       (sentiment/most-miserable-text)
       ;(flickr/search)
       ;(rand-nth)
       ;(flickr/get-photo)
...
Although not visible on the Guardrian Illustrated workflow visualisation the workflow uses the same piece of logic twice to select the most miserable text.
